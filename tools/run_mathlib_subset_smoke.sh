...
mkdir -p "$OUT_ROOT"
exec > "$OUT_ROOT/mathlib4_v420_subset_smoke.log" 2>&1
...
python experiments/trace_mathlib_subset_with_leandojo.py \
  ...
  --max-files 1 \
  --skip-cache-get \
  --inventory-jsonl "$OUT_ROOT/mathlib4_v420_subset_smoke_inventory.jsonl" \
  --selected-files-json "$OUT_ROOT/mathlib4_v420_subset_smoke_selected_files.json"
